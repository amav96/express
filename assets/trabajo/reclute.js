...
function sendDataFirstStep(nombre,email,pass,tipo,getLocateOnEstep,getPostalCodeOnEstep,
  ...
  ("0" + hoy.getDate()).slice(-2)+ ' ' + getHora + ':' + getMinutos + ':' + getSegundos;
      ...
      $.ajax({
          ...
          type:"POST",
          data:{nombre,email,pass,tipo,fecha,getLocateOnEstep,getPostalCodeOnEstep,
            textpais,textprovincia},
          beforeSend: function(){
...
$(document).on('click','#primer-siguiente',function(){
  ...
  if (domicilio === '') {
    ...
    if(pais.value === 'Argentina'){
      showErrorMonotribute('Debe ingresar CUIT','block','n');  prevSecondPage(); return
    }
    if(pais.value === 'Uruguay'){
      alertNegative("Debes ingresar el RUT")
      prevSecondPage(); return
    }
...
async function validateDocumentCuit(item){
 await validateExistCuit(item)
    ...
    if(resultValidate){
      if(pais.value === 'Argentina'){  
        if(item === ''){
            showErrorMonotribute('Debe ingresar CUIT','block','n'); return
        }else{
          if(pais.value === 'Argentina'){
          await validateCuit(item)
            .then(res => {
              if(res !== 'success'){
                showErrorMonotribute(res,'block','n'); return
              }
              showErrorMonotribute('','none','s'); return
            })
            .catch(err => {
              showErrorMonotribute(err,'block','n'); return
            })
          }
        }
      } 
      ...
      if(pais.value === 'Uruguay'){
          if(item === ''){
            ...
            if(pais.value === 'Uruguay'){
              ...
              .then(res =>{
                if(!res){
                  showErrorRut(' '+ ' '+ ' | '+'Rut inválido','inline','n'); return
                }
                showErrorRut('','none','s'); return
              })
              .catch(err => {
                showErrorRut(err,'inline','n'); return
              })
            }
          }
      }
...
//     monotributo : $("#monotributo").val(),
//     imgMonotributo : $("#imgMonotributo").val(),
//     imgDocumentoFrontal : $("#imgDocumentoFrontal").val(),
//     imgDocumentoDorsal : $("#imgDocumentoDorsal").val(),
//     imgCuilRut : $("#imgCuilRut").val(), 
//     imgComprobanteDomicilio : $("#imgComprobanteDomicilio").val(),
//     imgFrontalPersona : $("#imgFrontalPersona").val(),
//     telefono_celular : $("#telefono_celular").val(), 
//     via_conocimiento : $("#via_conocimiento").val(),
//   }
// console.log(send)
