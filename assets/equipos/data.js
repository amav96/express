...
//  if (inicialesEmpresa === "PS") {
  ...
  if (contSerieEnBase.is(":visible") || serieEnBaseINPUT.is(":visible")) {
    ...
    if (serieEnBaseINPUT.val() === "") {
         if(inicialesEmpresa === 'MV'){
          alertNegative('Debes ingresar nro de MAC'); return false;
         }

      alertNegative('Debes ingresar la serie del equipo'); return false;

    }else if (serieEnBaseINPUT.val().length < 5 && inicialesEmpresa != 'MV') {
...
// }
if(inicialesEmpresa === "IP" || inicialesEmpresa == "MV"){
  ...
  if(inicialesEmpresa === 'MV'){
    ...
    if (serieEnBaseINPUT.val() === "") {
      alertNegative('Debes ingresar la serie del equipofd'); return false;
    }
...
$("#enviarInfoNuevo").click(function(e){
  ...
  if(apellidoNuevoINPUT=== ""){
    ...
    alertNegative('Debes ingresar la serie/mac del equipos'); return false
...
$("#enviarAConsignacion").click(function(e){
    ...
    alertNegative('Debes ingresar la serie/mac del equiposd'); return false
    }else if(modeloNuevoINPUT=== ""){
    alertNegative('Debes ingresar el modelo del equipo'); return false
    }
    else {
    ...
    $.ajax({
      ...
      if(reciboRespuestaDeConsignacionCliente[0].result !== true){
        ...
        alertNegative("No pudo ser agregado con éxito!"); return false
...
$(document).on('change','#estado-rec-en-base',function(){
    ...
    componentes.show();
      ...
      if(identificacionCliente === 'IP' || identificacionCliente === 'MV'){
      if(estadoNotificacion !== 'RECUPERADO'){
...
async function getLocation() {
  ...
  navigator.geolocation.getCurrentPosition(pos => {
      resolve(pos);
      // esto retorna la ubicacion
  }, err => {
  
    alertInfo('Ubicación denegada','Para poder gestionar activá tu ubicación siguiendo estos pasos : Ingresa en tu navegador Chrome/Firefox : 1) Clic en Configuracion. 2) Configuracion avazanda / Configuracion de sitios. 3) Ubicación 4) Bloqueados ( Si devuelvoya.com esta en esta lista de bloqueados, clic en Permitir) y vuelve a cargar la web','info')
  });
...
async function locateMe() {
  ...
  locationRE = await this.getLocation();
  
  } catch(e) {

     alertNegative('Tipo de error: '+e)
