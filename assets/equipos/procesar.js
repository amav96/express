    ...
    $(document).on("click", "#confirmar", function(e) {
        ...
        if (empresasLS.length !== 0) {
            if (empresasLS !== infoEquipo.identificacion) {
                ...
                if (inicialesEmpresa === "PS" || inicialesEmpresa === "MV" || inicialesEmpresa === "LA" || inicialesEmpresa === "CL") {
                    $("#abrirfirmar").show()
                    $("#crear-imagen").show()
                    $("#borrar-imagen").show()
...
`;
    } else if (nombreEmpresaCaja !== 'LA' && nombreEmpresaCaja !== 'PS' && nombreEmpresaCaja !== 'GC' && nombreEmpresaCaja !== 'MV' && nombreEmpresaCaja !== 'CL') {
...
`;
    ...
    } else if (nombreEmpresaCaja === 'MV' || nombreEmpresaCaja === 'CL') {
...
function leerLocalStorageAlRecargarPagina() {
    ...
    equiposLS.forEach(function(infoEquipo) {
        ...
        if (nombreEmpresaCajaRC !== 'PS' && nombreEmpresaCajaRC !== 'LA' && nombreEmpresaCajaRC !== 'GC' && nombreEmpresaCajaRC !== 'MV' && nombreEmpresaCajaRC !== 'CL') {
            row.innerHTML = `
            <div class="lista">
        <span><strong>Serie:</strong>  ${infoEquipo.serie} </span>
        <span><strong>Identificacion:</strong>  ${infoEquipo.identificacion} </span>
        ...
        if (nombreEmpresaCajaRC === 'MV' || nombreEmpresaCajaRC === 'CL') {
            row.innerHTML = `
            <div class="lista">
        <span><strong>MAC:</strong>  ${infoEquipo.serie} </span>
        <span><strong>Identificacion:</strong>  ${infoEquipo.identificacion} </span>
...
function siHayUnEquipoAgregadoALacaja() {
    ...
    if (obtenerEquiposLocalStorage().length === 0 || obtenerEquiposLocalStorage().length == [] || obtenerEquiposLocalStorage() === null) {
        ...
        if (localStorage.getItem('en') === 'n') {
            ...
            if (iniEmp === 'PS' || iniEmp === 'MV' || iniEmp === 'CL' || iniEmp === 'LA') {
...
var registrarInactividad = function() {
    ...
    function reiniciarTiempo() {
        clearTimeout(t);
        t = setTimeout(tiempoExcedido, 10800000)
