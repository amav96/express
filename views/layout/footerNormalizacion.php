 <?php require_once 'views/modal/admin/modal_N_limpiar.php';?> 
 <?php require_once 'views/modal/admin/modal_N_base_original.php';?> 
 <?php require_once 'views/modal/admin/modal_mostrar_limpiador.php';?> 
 <?php require_once 'views/modal/admin/modal_importar_ordenador.php';?> 
 <?php require_once 'views/modal/admin/modal_mostrar_ordenador.php';?> 
 <?php require_once 'views/modal/admin/modal_importar_validador.php';?> 
 <?php require_once 'views/modal/admin/modal_mostrar_validador.php';?>
 <?php require_once 'views/modal/admin/modal_importar_limpiar_validados.php';?>
 <?php require_once 'views/modal/admin/modal_mostrar_limpiar_validados.php';?>
 <?php require_once 'views/modal/admin/modal_ingresar_base.php';?>
 <?php require_once 'views/modal/admin/modal_mostrar_base.php';?>
  
<script type="text/javascript" src="<?=base_url?>estilos/personal/datatables/JSZip-2.5.0/jszip.min.js"></script>
<script type="text/javascript" src="<?=base_url?>estilos/personal/datatables/pdfmake-0.1.36/pdfmake.min.js"></script>
<script type="text/javascript" src="<?=base_url?>estilos/personal/datatables/pdfmake-0.1.36/vfs_fonts.js"></script>
<script type="text/javascript" src="<?=base_url?>estilos/personal/datatables/DataTables-1.10.22/js/jquery.dataTables.min.js"></script>
<script type="text/javascript" src="<?=base_url?>estilos/personal/datatables/AutoFill-2.3.5/js/dataTables.autoFill.min.js"></script>
<script type="text/javascript" src="<?=base_url?>estilos/personal/datatables/Buttons-1.6.5/js/dataTables.buttons.min.js"></script>
<script type="text/javascript" src="<?=base_url?>estilos/personal/datatables/Buttons-1.6.5/js/buttons.html5.min.js"></script>
<script type="text/javascript" src="<?=base_url?>estilos/personal/datatables/Buttons-1.6.5/js/buttons.print.min.js"></script>
<script type="text/javascript" src="<?=base_url?>/estilos/personal/dataTables_adm/main.js"></script>  

      
     <!-- código JS propìo-->    
     
     
     <script src="<?=base_url?>assets/admin/normalizacion.js"></script> 
     <script src="<?=base_url?>assets/admin/panel.js?v=05042021"></script>
     <script src="https://cdn.jsdelivr.net/npm/sweetalert2@9"></script> 

</body>

</html>